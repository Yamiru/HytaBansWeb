...
<div class="admin-dashboard">
    ...
    <ul class="nav nav-tabs mb-4" id="adminTabs" role="tablist">
        ...
        <li class="nav-item" role="presentation">
            <button class="nav-link" id="staff-tab" data-bs-toggle="tab" data-bs-target="#staff" type="button">
                <i class="fas fa-user-shield"></i> Staff Stats
            </button>
        </li>
       <?php if (($currentUser['role'] ?? 'admin') === 'admin'): ?>
        <li class="nav-item" role="presentation">
    ...
    <div class="tab-content" id="adminTabContent">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <table class="table table-sm">
                                <tr>
                                    <td class="text-muted">HytaBansWeb Version</td>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" name="site_name" 
                                           value="<?= htmlspecialchars($config['site_name'] ?? 'HytaBans', ENT_QUOTES, 'UTF-8') ?>">
                                </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" name="site_keywords" 
                                           ...
                                           placeholder="minecraft, hytabans, punishments, bans, server">
        ...
        <!-- Staff Statistics Tab -->
        <div class="tab-pane fade" id="staff" role="tabpanel">
            <div class="card">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        <h5 class="mb-0"><i class="fas fa-user-shield text-primary"></i> Staff Statistics</h5>
                        <button class="btn btn-sm btn-outline-secondary" id="refresh-staff-stats">
                            <i class="fas fa-sync-alt"></i> Refresh
                        </button>
                    </div>
                    
                    <div id="staff-stats-content">
                        <div class="text-center py-4">
                            <div class="spinner-border text-primary" role="status">
                                <span class="visually-hidden">Loading...</span>
                            </div>
                            <p class="mt-2 text-muted">Loading staff statistics...</p>
                        </div>
                    </div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (settingsForm) {
        settingsForm.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                if (result.success) {
                    // Show success message and always reload to apply changes
                    const saveBtn = settingsForm.querySelector('button[type="submit"]');
                    if (saveBtn) {
                        saveBtn.innerHTML = '<i class="fas fa-check"></i> Saved! Reloading...';
                        saveBtn.disabled = true;
                    }
                    // Always reload page after successful save
                    setTimeout(() => {
    ...
    let staffStatsLoaded = false;
    
    // Load staff stats when tab is shown
    ...
    // Refresh button
    document.getElementById('refresh-staff-stats')?.addEventListener('click', function() {
        loadStaffStats();
    });
    ...
    async function loadStaffStats() {
        ...
        try {
            const response = await fetch('<?= url('admin/staff-stats') ?>');
            const data = await response.json();
            
            if (data.success && data.stats) {
                renderStaffStats(data.stats);
                staffStatsLoaded = true;
            } else {
                container.innerHTML = '<div class="alert alert-warning">Failed to load staff statistics</div>';
            }
        } catch (error) {
    ...
    function renderStaffStats(stats) {
        ...
        const totals = stats.reduce((acc, s) => {
            ...
            acc.kicks += s.kicks || 0;
            acc.last_7d += s.last_7d || 0;
            acc.last_30d += s.last_30d || 0;
            return acc;
        }, { bans: 0, mutes: 0, warnings: 0, kicks: 0, last_7d: 0, last_30d: 0 });
        
        let html = `
            <div class="row g-3 mb-4">
                <div class="col-md-3"><div class="card bg-danger bg-opacity-10 border-danger"><div class="card-body text-center py-3"><h4 class="text-danger mb-0">${totals.bans}</h4><small>Total Bans</small></div></div></div>
                <div class="col-md-3"><div class="card bg-warning bg-opacity-10 border-warning"><div class="card-body text-center py-3"><h4 class="text-warning mb-0">${totals.mutes}</h4><small>Total Mutes</small></div></div></div>
                <div class="col-md-3"><div class="card bg-info bg-opacity-10 border-info"><div class="card-body text-center py-3"><h4 class="text-info mb-0">${totals.warnings}</h4><small>Total Warns</small></div></div></div>
                <div class="col-md-3"><div class="card bg-secondary bg-opacity-10 border-secondary"><div class="card-body text-center py-3"><h4 class="text-secondary mb-0">${totals.kicks}</h4><small>Total Kicks</small></div></div></div>
            </div>
            <div class="table-responsive">
                <table class="table table-hover">
                    <thead><tr>
                        <th>#</th><th>Staff Member</th>
                        <th class="text-center">Bans</th><th class="text-center">Mutes</th>
                        <th class="text-center">Warns</th><th class="text-center">Kicks</th>
                        <th class="text-center">Total</th><th class="text-center">7d</th>
                        <th class="text-center">30d</th><th>Last Active</th>
                    </tr></thead><tbody>
        `;
        
        stats.forEach((staff, index) => {
            const lastActive = staff.last_action ? new Date(staff.last_action * 1000).toLocaleDateString() : 'Never';
            html += `
                <tr>
                    <td><span class="badge bg-primary">${index + 1}</span></td>
                    <td><i class="fas fa-user-shield text-primary me-2"></i><strong>${escapeHtml(staff.staff_name)}</strong></td>
                    <td class="text-center"><span class="badge bg-danger">${staff.bans || 0}</span></td>
                    <td class="text-center"><span class="badge bg-warning">${staff.mutes || 0}</span></td>
                    <td class="text-center"><span class="badge bg-info">${staff.warnings || 0}</span></td>
                    <td class="text-center"><span class="badge bg-secondary">${staff.kicks || 0}</span></td>
                    <td class="text-center"><strong>${staff.total || 0}</strong></td>
                    <td class="text-center">${staff.last_7d || 0}</td>
                    <td class="text-center">${staff.last_30d || 0}</td>
